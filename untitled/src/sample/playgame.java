...
public class playgame {
    private int SKORE=0;
    public Scene SHOW(ball b1,Stage MainStage,PauseScreen PAUSED,SavedGameScreen SAVEDGAMES,HighScoreScreen HIGHSCORE,HomeScreen HOME,Obstacle ob[],ObstacleHitScreen HIT,int SKORE2) {
        SKORE=SKORE2;
        ...
        pause_v.setX(800);
        pause_v.setY(30);
        roo2.getChildren().add(pause_v);
        roo2.getChildren().add(b2);
        pause_v.setOnMouseClicked((MouseEvent e) -> {
            try {
                r[1]=0;
                MainStage.setScene(PAUSED.SHOW(b1.clon(),this,MainStage,SAVEDGAMES,HIGHSCORE,HOME,ob.clone(),HIT,SKORE));
            } catch (Exception exception) {
                exception.printStackTrace();
            }
        });
        ArrayList<Group> grouplist = new ArrayList<Group>();
        javafx.scene.image.Image rewardd=new javafx.scene.image.Image("reward.jpg");
        ...
        Scene scene = new Scene(roo2, bounds.getWidth(), bounds.getHeight(), Color.rgb(41, 41, 41));
                ...
                if (r[1]==1) {
                    ...
                    if (b1.getYcord() >= 650) {
                        try {
                            r[1]=0;
                            this.stop();
                            MainStage.setScene(HIT.SHOW(new ball(), MainStage, SAVEDGAMES, getPlay(), HIGHSCORE, HOME, PAUSED, ob,SKORE));
                        } catch (Exception exception) {
                            exception.printStackTrace();
                            this.stop();
                        }
                        ...
                        for (Node path: obs[i].getChildren()){
                            if (((Path)Shape.intersect(circle, (Path)path)).getElements().size()>0 ){
                                ...
                                if (test==false)
                                {
                                    try {
                                        this.stop();
                                        MainStage.setScene(HIT.SHOW(b1, MainStage, SAVEDGAMES, getPlay(), HIGHSCORE, HOME, PAUSED, ob,SKORE));
                                        break;
                                    } catch (Exception e) {
                                        e.printStackTrace();
                                    }
...
//                                settingSkore(score[0]);
                                b2.setText("SCORE: "+ SKORE);}
                        }
                        if (ob[i].getYcord()>1200)
                        {
                            System.out.println("before play "+r[3]);
                            ob[i].setYcord(ob[r[3]].getYcord()-500);
                            r[3]=i;
                            System.out.println("after play"+r[3]);
    ...
    private playgame getPlay(){return this;}
...
//            {
//                Path path=new Path();
//                if(i<2){path.getElements().add(new MoveTo(xcen - obj.getInner_y2(), ycen - 100));}
//                else{path.getElements().add(new MoveTo(xcen - obj.getInner_y2(), ycen + 100));}
//
//            }
            Path path1=new Path();
            Path path2=new Path();
